                    ...
                    if ((szDirs = Directory.GetDirectories(szBaseDir)) != null && szDirs.Length > 0)
                        foreach (string szDir0 in szDirs)
                            if (recursive)
                                ...
                                entries.Add(new LSFileInfo(new DirectoryInfo(szDir0)));
                } catch (Exception ex) {
                    Trace.WriteLine(ex.Message);
                }
                try {
                    if ((aszFiles = Directory.GetFiles(szBaseDir,szWildcard)) != null && aszFiles.Length > 0)
                        foreach (string aFile in aszFiles)
                                entries.Add(new LSFileInfo(new FileInfo(aFile)));
                } catch (Exception ex) {
                    Trace.WriteLine(ex.Message);
                }
#else
