...
public class main {
    ...
    static void doArg(string szArg, bool bSub, bool bAll, bool bSortSize, bool bDirsOnly) {
    ...
    static List<string> badDirs = new List<string>();

    static ArrayList calcDirSize(string szDir, string szBaseDir, bool bAll, out long lRet) {
        long lTmp;
        int nLen;
        ...
        try {
            ...
            if (!badDirs.Contains(szDir))
                badDirs.Add(szDir);
            else
                Trace.WriteLine("already exists in list: " + szDir);
        }
        return ret;
    }

    static void showContents(string szDir, bool bSub, bool bAll, bool bSortSize, bool bDirsOnly) {
